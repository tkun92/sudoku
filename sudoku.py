                ...
                for row_index, row in enumerate(self._table):
                    for col_index, cell in enumerate(row):
                        if cell.cube_id == cube_id and (
                                row_index, col_index) not in same_indices and number in cell.possibilities and len(
...
if __name__ == "__main__":
    ...
    for counter, item in enumerate(listsudoku):
        ...
        sudoku.check_table()
        #print("&&&&&&&&")
        #print(sudoku.isitdone())
        output.writelines("Counter:" + str(counter) +"\n")
        output.writelines(str(sudoku.isitdone()) + "\n")
        for i in range(9):
            for j in range(9):
